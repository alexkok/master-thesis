...
% RQ Main \ How can we automatically test the implementation of the generated program?
 ...
 % RQ 2 \item What kind of errors can we find using this approach?
  ...
  % RQ 3 \item How can we extend our test suit to find more implementation errors?
  % - Add sync block
...
% New
\def \rqMain{
	How can we automatically test the generator, and thus the generated system, to check whether the implementation works as expected?
}
...
% New
\def \rqOne{
	Which properties are expected to hold on the generator?
}
% Old one: zoom in, # of bugs and which type?
%\def \rqTwo{ What kind of errors can we find using this approach? }
% New
\def \rqTwo{
	How can we automatically test each property to find as much as possible bugs?
}
...
\input{content/ch7_discussion.tex}
\input{content/ch8_conclusion.tex}
\input{content/ch9_end.tex}
