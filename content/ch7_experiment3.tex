...
\\
A custom generator is being created that uses the preconditions to determine the
input values. Note that this can be seen as an update to the generator that was
...
matching the condition can become complex. With expressions, we mean a
combination of operators with literals and variables. Instead, the resulting
value generator requires having at least a variable on one side of the
expression.\\
\\
...
variable (\autoref{lst:experiment3_value_generation_code},~Line~7). Setting the
minimum and maximum value to a default value and allowing zero by default.
...
Each statement in the preconditions block is being checked
(\autoref{lst:experiment3_value_generation_code},~Line~9 -13). The
...
(\autoref{lst:experiment3_value_generation_code},~Line~15-18).
...
input values (\autoref{lst:experiment3_value_generation_code},~Line~21). In the
end, the list is being returned, containing all the generated input values that
match the preconditions of the event.
