...
The properties using division are now using the \code{round()} method in its definition. \textit{Rebel} does not provide a way to round a value, which is why we need to define the function in the specification. In \textit{Rebel} a function is defined as an expression that is being executed whenever the function is being called. To implement the rounding \todo{HERE} functionality, such that the generated system makes use of this new function, we return the method body as a \textit{String}. Next, we modify the generator such that it generates the method with this body, such that the method also exists in the generated system. As mentioned earlier, \textit{Rebel} doesn't have the functionality to round a value, which is why a modification is needed in order to be able to test the division properties that we have defined.\\
\\
