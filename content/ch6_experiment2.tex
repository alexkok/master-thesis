...
experiment we found that the implicative properties were not tested thoroughly.
In this experiment these properties are triggering the if-clause of the
properties using implication, thus we expect the test coverage to be higher when
looking at the coverage of the properties. This also follows from the results,
as we can see in \autoref{fig:experiment2_eval_e2}, the test coverage when
looking at the logic file of the implicative properties is 74\%.
% Figure
\FloatBarrier
\begin{figure}[!ht]
%\frame{
...
happen in this case. The definitions of the division property will probably need
to be updated too. In the next experiment we will focus on improving this.

% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% Section: Threats to validity
\section{Threats to validity}

\todo{Add threat: Running multiple times sometimes causes invalid values when using GT and LT. High probability that this is caused by the random operation. (can be added to the point below here)}
