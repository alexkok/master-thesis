...
is to test the implementation of the generator and trying to find yet unknown
...
We described the folloring 5 phases that are required to test the generator:
\begin{enumerate}
\item \tfPhaseOne{}
\item \tfPhaseTwo{}
\item \tfPhaseThree{}
...
\section{Threats to validity}
...
% - Not compilable system. Can be detected but not sure if its a bug or soemthing else that went wrong. Hard to argua, which is why we assumed it to be compilable. However, this might happen when running the test suite if certain operations that are expected to work are incorrectly being translated.
