...
The field properties can be compared to the field axioms on real numbers as
described in~\cite{apostol2007calculus}:
\begin{quote}
...
\end{quote}
The \textit{addition} and \textit{multiplication} operations are also available
when using the \textit{Money} type in \textit{Rebel} and are expected to apply
these operations on the amount value. However, not every combination is possible
in \textit{Rebel} when it comes to using these operations, as described earlier.
The following properties are using \textit{addition} and/or
\textit{multiplication} in their definitions.
\todo{Add footnote noting that: we do not claim to have the exact field properties on the money type, as that is not possible in rebel. Instead, we use these definitions together with other types to define the properties.}
\todo{Move to here: Associativity, Commutativity, Identity, Inverse, Distributivity}
...
\todo{Move to here: Others}

% % % % % % % % % % % % %
% Subsection: Additional properties of equality and inequality
\subsection{Additional properties of equality and inequality}
\label{ssct:properties_definitions_additionalproperties}
For the third experiment (\autoref{cpt:experiment3}), additional properties have
been defined to the list. These can be put in the category of equality and
inequality properties and are using the implication ($\implies$) sign in its
definitions.

\todo{Add: subtraction, multiplication, addition+ (Inequality), divsion+}
\subsubsection{Division}
Due to the second experiment, the property definition for \textit{Division} had
to be updated, as it did not take the division problem into account. It now
makes use of a method \code{round()}, which rounds the value to 4 decimals. The
rounding method used for this is ``HALF\_UP'', because rounding the value down
might still cause a failure when using big numbers\todo{Reasoning? Why HALF-UP exactly?}.\\
\\
In addition to the updated definition, more combinations for the division
property are defined. In ...\todo{Add table} we show the updated definition,
along with the added definitions for the division property.

% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% % % % % % % % % % % %   EXISTING  % % % % % % % % % % % % % % % % % % % % %
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
\todo[inline]{Put the existing properties into the right category.
And fix references among them}
